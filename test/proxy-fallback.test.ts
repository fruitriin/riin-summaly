...
describe('scpaping forceProxyFallback (phase12.6)', () => {
	let mockProxy: Server;
	let mockProxyUrl: string;
	let mockProxyHits = 0;
	...
	beforeEach(async () => {
		mockProxyHits = 0;
		mockProxy = createServer((_req, res) => {
			mockProxyHits++;
			res.writeHead(200, {
				'content-type': 'text/html; charset=utf-8',
				'x-summaly-final-url': 'https://store.jp.square-enix.com/item/MWFF140773_2.html',
			});
			res.end('<html><head><title>Force Proxy Direct</title></head><body>x</body></html>');
		});
		await new Promise<void>(resolve => mockProxy.listen(0, '127.0.0.1', resolve));
		const addr = mockProxy.address() as AddressInfo;
		mockProxyUrl = `http://127.0.0.1:${addr.port}`;
	...
	function makeProxyCfg(overrides: Partial<ProxyFallbackConfig> = {}): ProxyFallbackConfig {
		return {
			enabled: true,
			url: mockProxyUrl,
			secret: 'test-secret',
			categories: ['origin_error', 'bot_blocked'],
			domains: ['store.jp.square-enix.com'],
			timeoutMs: 5000,
			...overrides,
		};
	}

	test('forceProxyFallback: true + proxy 有効 + allowlist 一致なら 1〜2段目をスキップして直接 proxy を呼ぶ', async () => {
		const { scpaping } = await import('@/utils/got.js');
		// 通常段階に流れたら DNS で即 fail する non-existent host を使う。proxy 直行が動けば mock proxy が成功を返す。
		const result = await scpaping('https://store.jp.square-enix.com/item/MWFF140773_2.html', {
			proxyFallback: makeProxyCfg(),
			forceProxyFallback: true,
		});
		expect(result.response.statusCode).toBe(200);
		expect(result.body).toContain('<title>Force Proxy Direct</title>');
		expect(result.$('title').text()).toBe('Force Proxy Direct');
		expect(mockProxyHits).toBe(1);
	});

	test('forceProxyFallback: true でも proxy が enabled = false なら通常段階に fallthrough', async () => {
		const { scpaping } = await import('@/utils/got.js');
		const cfg = makeProxyCfg({ enabled: false });
		await expect(
	...
	test('forceProxyFallback: true でも secret 空なら通常段階に fallthrough', async () => {
		const { scpaping } = await import('@/utils/got.js');
		const cfg = makeProxyCfg({ secret: '' });
		await expect(
			scpaping('https://nonexistent-host.invalid/page', {
				proxyFallback: cfg,
				forceProxyFallback: true,
	...
	test('forceProxyFallback: true でも domain allowlist 不一致なら通常段階に fallthrough', async () => {
		const { scpaping } = await import('@/utils/got.js');
		const cfg = makeProxyCfg({ domains: ['only-allowed.example'] });
		await expect(
			scpaping('https://nonexistent-host.invalid/page', {
				proxyFallback: cfg,
				forceProxyFallback: true,
	...
	test('forceProxyFallback: true でも http: (非 https) なら通常段階に fallthrough', async () => {
		const { scpaping } = await import('@/utils/got.js');
		const cfg = makeProxyCfg({ domains: ['nonexistent-host.invalid'] });
		await expect(
			scpaping('http://nonexistent-host.invalid/page', {
				proxyFallback: cfg,
				forceProxyFallback: true,
			}),
		).rejects.toThrow();
