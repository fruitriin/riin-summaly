...
/**
 * Square Enix e-STORE (`store.jp.square-enix.com`) のプラグイン (phase12.6 → phase14 Step 4
 * で経路学習キャッシュ + bootstrap に統合)。
 *
 ...
 * `store.jp.square-enix.com → proxy` のエントリが入ったため、Fastify モードで cache 有効なら
 * `scpaping()` 冒頭の cache hit fast path で直接 proxy が呼ばれる。本プラグインは特殊な経路強制を
 * 一切行わず通常の cascade に乗る。
 *
 ...
 *
 * **strategy_cache 無効環境のデグレ警告 (W-1 review feedback)**: 上記要件を満たさない場合
 * (strategy_cache 無効、または bootstrap が連続失敗で破棄された後、または proxy が未設定)、
 * cache fast path が発火しないため通常 cascade に乗り、HTTP 200 + 正規 404 ページボディを
 * 取得して isThinSummary 判定で thin → 失敗と扱われる (破壊的ではない)。本プラグインは
 * Fastify モード + strategy_cache 有効 + proxy 設定済を前提としており、library 直接利用や
 * proxy 未設定環境では期待通りに動かない。
 */
