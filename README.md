...
| `google-drive` | `drive.google.com/file/d/<id>/...` | URL 直接組み立て + thumbnail/OGP | 共有ファイルの動画 / PDF / 画像 / Docs をインライン表示。**embed 有効時は自前の HTML5 `<video>` プレイヤー** (Drive 直ストリーミング URL を `confirm`/`uuid` 解決して `<video src>` に流す。CORS + Range 対応でブラウザ↔Drive 直再生、summaly は中継しない。スマホでコントロールが崩れない)、embed 無効時は Drive 公式 `…/preview` iframe にフォールバック。公開 thumbnail の pixel 寸法から **実アスペクト比を判定** (縦動画は縦長)、`/view` OGP から file 名を title 補完。`skipRedirectResolution = true`。**Google フォト (`photos.google.com`) は `X-Frame-Options: SAMEORIGIN` で iframe 不可のため非対応** (phase19.1) |
