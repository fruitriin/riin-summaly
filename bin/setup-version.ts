 ...
 * ビルド時定数。tsx で TS を直接実行する `bin/summaly-server.ts` ではビルドステップを通らないため、
 ...
 * 後続の `../src/index.js` 経由で `src/utils/got.ts` や `/v` エンドポイントが評価される時点で全定数は定義済み。
 *
 * `.git` が無い環境では git 情報は 'unknown' にフォールバックする。
 */
