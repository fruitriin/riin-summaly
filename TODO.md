...
## 現在のフェーズ: phase14 Step 3 (bootstrap JSONL 同梱) 完了 (2026-05-08)。Step 4 (forceX 廃止) 〜 Step 7 が残
...
| 高 | 14 | [docs/plans/phase14-domain-strategy-cache.md](docs/plans/phase14-domain-strategy-cache.md) — 経路学習キャッシュ (host + path prefix 2段、JSONL 永続化、N 連続失敗で invalidate)。bootstrap JSONL 同梱で初回コスト回避。`forceCurlCffiFallback` / `forceProxyFallback` を廃止し、プラグインは「引き出し方の自在性」専用に整理。汎用パスでも自動最適化される | M〜L | 進行中 (Step 1 + Step 2 系 + Step 3 bootstrap 同梱 完了 2026-05-08、Step 4 forceX 廃止 / Step 5 dev UI / Step 6 docs / Step 7 skill 更新 が残) |
...
phase14   進行中（Step 1 + Step 2 系 + Step 3 bootstrap 同梱 完了 2026-05-08、Step 4 forceX 廃止 〜 Step 7 残）
```
