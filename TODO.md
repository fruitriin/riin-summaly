...
| 高 | 15.4 | [docs/plans/phase15.4-plugin-nitori.md](docs/plans/phase15.4-plugin-nitori.md) — ニトリ (nitori-net.jp) プラグイン。公式 JSON API (`/occ/v2/nitorinet/nitori/products/<sku>`) + curl_cffi 経路で TLS layer + UA layer の二重 bot block を迂回。fail mode I (SPA + JS 動的 OGP) に整理されていたが、隠れ JSON API 発見で救援可能と判明。`viaCurlCffi` 直接呼び (個別 hardcode 方式)、`skipRedirectResolution = true`。**Followup #1 (2026-05-10)**: 本番で HTTP/2 INTERNAL_ERROR が観測 → curl_cffi CLI が headers を受け取らない設計だったため Accept ヘッダ上書きが効かなかった root cause を特定。CLI に `--header NAME:VALUE` 機構追加 + `viaCurlCffi` から allowlist (`accept` / `accept-language` / `referer` / `user-agent`) で渡す経路を実装、ローカル検証で JSON 200 OK 確認 | M | 本番動作確認待ち (2026-05-10、Followup #1 デプロイ後 datacenter IP block の有無で B 系オプション (proxy 経由 / 複合経路) 検討) |
