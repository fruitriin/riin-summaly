...
## 現在のフェーズ: phase12.1 (Cloudflare Workers proxy fallback、Amazon class IP block 救援) — 実験ステップ含む
...
| 高 | 12.1 | [docs/plans/phase12.1-cf-workers-proxy-fallback.md](docs/plans/phase12.1-cf-workers-proxy-fallback.md) — Cloudflare Workers Free を outbound proxy として使い、Amazon class の IP block を救援。実験ステップ (Step 1.3) で GO/NO-GO 判定する設計 | M〜L | 進行中（Step 1.1〜1.2 完了、**Step 1.3 オーナー実機検証待ち**） |
...
phase11.9 完了（bot block UA リトライ、pino fallback フィールドは phase11.6 に廆す）
   ↓
phase12.1 進行中（CF Workers proxy fallback、Step 1.1〜1.2 完了、Step 1.3 オーナー実機検証待ち）
```
