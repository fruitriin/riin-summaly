...
Plan: [docs/plans/phase16.4-startup-healthcheck.md](../docs/plans/phase16.4-startup-healthcheck.md)

- [x] Step 1: `bin/healthcheck.ts` 新設 (proxy / curl_cffi / embed の placeholder + 実体検証)
- [x] Step 2: `bin/summaly-server.ts` から起動時呼び出し
- [x] Step 3: `test/healthcheck.test.ts` 14 件 pass
- [x] Step 4: `config.example.toml` 基本方針コメント追記 + `CHANGELOG.md` エントリ + Plan 完了状況
- [x] Step 5: 品質ゲート全パス (build / lint / typecheck / test 573 件)
- [ ] アーカイブ + コミット
