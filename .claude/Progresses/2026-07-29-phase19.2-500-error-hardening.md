...
### phase19.2 — 本番 500 エラーの根絶 (docs/plans/phase19.2-500-error-hardening.md)

- [x] Step 1: Bug A 修正 — getOEmbedPlayer の `body.html` 型ガード
  - [x] src/general.ts L42 に `typeof body.html !== 'string'` 追加
  - [x] html 欠落 oEmbed フィクスチャテスト (player=null + 500 にならない)
- [x] Step 2: Bug B 修正 — parseGeneral 内 URL 解決の安全化
  - [x] `tryResolveUrl()` ヘルパ追加
  - [x] L337 (image) / L72 (oEmbed src) / L398 (favicon) 置き換え
  - [x] 壊れた og:image フィクスチャテスト
- [x] Step 3: categorizeError 拡張
  - [x] `tls_error` カテゴリ新設 + パターン追加 (証明書 / SSL routines / EPROTO)
  - [x] curl_cffi HTTP/2 INTERNAL_ERROR → connection_dropped 振り分け
  - [x] LOG_LEVEL_BY_CATEGORY / FILTERED_CATEGORIES / VALID_ERROR_CATEGORIES 同期
  - [x] DEFAULT_FALLBACK_RETRY_CATEGORIES / HEDGED_FINAL_CATEGORIES への影響判断
  - [x] unit test (パターン → カテゴリ → レベル)
- [x] Step 4: ドキュメント同期 (Library.md / CHANGELOG / config example コメント)
- [x] Stage 1: build / eslint / typecheck / test + addf-lint
- [x] Step 4.6: ノウハウ再確認 + 自己レビュー
- [x] Stage 2: addf-code-review-agent → 指摘対応
- [x] 完了処理 (knowhow 記録 / Plan 反映 / Feedback / アーカイブ / コミット)
