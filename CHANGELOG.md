(unreleased)
------------------
* **feat (plugin: google-drive / embed)**: Google Drive に **自前の HTML5 `<video>` プレイヤー**を追加 (phase19.1 followup #3)。背景: Drive の `/preview` iframe player はスマホ幅でコントロール UI が崩れる (Drive `/preview` を直接開いても崩れる = Drive 側の問題で iframe 比率調整では直せないと実機切り分け)。対策: embed 有効時 (`embedBaseUrl` 設定) は `player.url` を Drive iframe ではなく summaly の `/embed?url=...` に向け、`renderEmbed` で **Drive 直ストリーミング URL** (`drive.usercontent.google.com/download?id=<id>&export=download&confirm=t&uuid=...`) を `<video controls playsinline>` に流したレスポンシブ HTML を返す。この download URL は `Access-Control-Allow-Origin: *` + `Accept-Ranges: bytes` を返すため第三者サイトの `<video>` から CORS + seek 再生可能 (実機確認、206 Partial Content)。大きい file の「ウイルススキャン確認」HTML は新規 `src/utils/drive-download.ts` の `resolveDownloadUrl` が `confirm`/`uuid` を抽出して解決 (gdown と同方式)。HTML 生成は `src/utils/drive-video-embed.ts` の `composeDriveVideoEmbedHtml` (pure、`<script>` なし、URL は `https:` のみ + escapeHtml)。embed の CSP に `media-src` を許可するため `EmbedRenderResult.mediaSrc?: string[]` を新設し、embed エンドポイント (本番 `src/index.ts` + dev `dev/server.ts`) で **origin-only 再検証**して `media-src` に反映 (CSP ヘッダインジェクション防御)。**バイナリは summaly が中継しない** (URL を渡すだけ、再生はブラウザ↔Drive 直、帯域負担ゼロ)。embed 無効 (library mode 等) では従来通り Drive `/preview` iframe にフォールバック (`composePlayerUrl` で分岐)。自前 video 経路では実アスペクト比をそのまま使い (コントロールバー余白なし)、iframe フォールバック経路のみ `withControlBar` で余白加算。`resolveConfirmUrlFromHtml` / `composeDriveVideoEmbedHtml` / `pickHttpsUrl` / `composePlayerUrl` の単体テスト 9 件追加 (計 696 件 pass)
