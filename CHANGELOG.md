(unreleased)
------------------
* **feat (plugin: spotify)**: `description` フィールドにアーティスト名を含めるよう拡張。Spotify の oEmbed API にはアーティスト情報が含まれないため、`facebookexternalhit` UA でページ本体を取得（nintendo-store と同型の SNS bot UA allowlist パターン）し、`music:musician_description` メタタグ（無ければ `og:description` の先頭セグメント）から抽出する。`og:type` が `music.song` / `music.album` のときのみ抽出（`music.playlist` / アーティスト自身のページは対象外）。oEmbed とページ本体の取得は `Promise.all` で並行発火してレイテンシを抑制し、ページ本体取得が bot block 等で失敗しても oEmbed ベースの summary（`description: null`）を返すフェイルセーフを維持。`extractArtist($: cheerio.CheerioAPI): string | null` を pure 関数として export、単体テスト 4 件追加。**レビュー対応**: 補助 fetch は track / album パスのみ発火（`isArtistBearingPath` export、`/intl-xx/` locale プレフィックス対応。playlist / artist / show 等での無駄 fetch を排除）、短縮タイムアウト（response 5s / operation 10s）で oEmbed 側のレイテンシを引きずらない、`proxyFallback` / `curlCffiFallback` / `_cacheRecording` を明示無効化して補助 fetch の失敗・hedge 発火が外部経路 quota 消費・経路学習キャッシュ汚染・pino hedge ログの誤帰属を起こさないよう本体経路から分離
* **fix (general)**: 本番 256 時間分のログ分析で判明した **summaly 自身に起因する 500 エラー 2 種を修正** (phase19.2)。(1) `getOEmbedPlayer` が oEmbed JSON の `html` フィールドを型ガードなしで `.startsWith()` していたため、`html` を持たない oEmbed 実装 (fixupx.com / unityroom.com / fxtwitter.com 等、実測 199 件 = error の 65%) で `TypeError` → 500 になっていた。`typeof body.html !== 'string'` を well-formed 判定に追加し player なし (OGP ベース card) にグレースフルデグレード。(2) `parseGeneral` の `og:image` / favicon href を無防備に `new URL()` へ渡していたため、壊れた URL 属性値を持つサイト (実測 6 件) で `TypeError: Invalid URL` → 500 になっていた。`tryResolveUrl()` ヘルパ (throw せず null) に置き換え、該当フィールドのみ null で処理続行
* **enhance (error category)**: `SummalyErrorCategory` に **`tls_error`** (証明書検証失敗: 期限切れ / self-signed / チェーン不完全 / altnames 不一致) を新設 (phase19.2)。証明書問題はどの取得経路 (got / CF Workers proxy / curl_cffi) でも救えない決定的失敗のため hedge fire skip 対象 (`HEDGED_FINAL_CATEGORIES`) に追加し、ログレベルは warn。また TLS/HTTP2 層の handshake 拒否 (`EPROTO` SSL alert / `secure TLS connection` 前切断 / curl HTTP/2 stream 切断) は curl_cffi 偽装で救援可能な bot block シグネチャとして `connection_dropped` に、`fetch failed` / `incorrect header check` / message 空の `RequestError` は `network_error` に、リダイレクトループ (`Redirected N times`) は `origin_error` に分類拡張。これにより外部要因の失敗が error レベルで記録されるノイズが解消され、**pino ログの error レベル = summaly 自身のバグ** という観測規約が回復する (実測: 256h で error 308 件 → 分類後は真のバグのみ)
