...
* **enhance (plugin: kakuyomu episode)**: episode (各話) URL のプレビュー識別性を改善。**card description**: 旧 `あらすじ: ... / 序章` (序章があらすじの一部に見える) → 新 `「序章」 / あらすじ: ...` (各話タイトルを prefix に置いて区別を明確化)。**embed UI**: 作品タイトル直下に `<div class="episode-title">「<各話タイトル>」</div>` を 0.95rem の太字で表示し、どの話か一目で分かる構造に。`composeDescription` / `composeEmbedHtml` のシグネチャに `episodeTitle: string \| null = null` (3 番目の引数) を追加、`renderEmbed` 側でも `extracted.episodeId != null` のとき `fetchEpisodeTitle` を並列取得して embed HTML に反映。XSS テスト含む 7 件追加
