...
| player (2 経路) | **embed 有効時 (Fastify モード + `embedBaseUrl` 設定)**: `player.url = <embedBaseUrl>/embed?url=<原 URL>` に向け、`renderEmbed` が **自前の HTML5 `<video controls>`** を返す (下記「自前 video プレイヤー」)。Drive `/preview` iframe を介さないため**スマホ幅でもコントロールが崩れない**。**embed 無効時 (library mode 等)**: Drive 公式 `https://drive.google.com/file/d/<id>/preview` iframe にフォールバック。`composePlayerUrl(url, id, embedBaseUrl)` で分岐 (pure export)。`allow` = `PLAYER_ALLOW_OEMBED` |
| 自前 video プレイヤー (`renderEmbed`) | Drive の **直ストリーミング URL** (`drive.usercontent.google.com/download?id=<id>&export=download&confirm=t&uuid=...`) を `<video src>` に流す。この URL は `Access-Control-Allow-Origin: *` + `Accept-Ranges: bytes` を返すため第三者サイトの `<video>` から CORS + seek 再生できる (実機確認 2026-06-01)。大きい file は「ウイルススキャン確認」HTML が挟まるので `src/utils/drive-download.ts` の `resolveDownloadUrl` が `confirm` / `uuid` を抽出して解決済み URL を組む (gdown と同方式)。HTML 生成は `src/utils/drive-video-embed.ts` の `composeDriveVideoEmbedHtml` (pure、`<script>` なし、URL は `https:` のみ + escapeHtml)。embed の CSP に `media-src https://drive.usercontent.google.com` を `EmbedRenderResult.mediaSrc` 経由で追加 (origin-only 再検証で CSP インジェクション防御)。**バイナリは summaly が中継しない** (URL を渡すだけ、再生はブラウザ↔Drive 直、帯域負担ゼロ) |
| アスペクト比 (縦動画対応) | **公開 thumbnail エンドポイント** `https://drive.google.com/thumbnail?id=<id>&sz=w1000` は file の実アスペクト比を保った画像を返す (縦動画なら縦長 JPEG)。これを取得して `src/utils/image-dimensions.ts` の `getImageDimensions` で pixel 寸法を読み、`player.width` / `player.height` に **実比率**を入れる。Misskey は height/width 比率で縦横比を計算するため、**縦動画は縦長プレビュー / 横動画は横長プレビュー**になる。取得失敗時は **16:9 にフォールバック**。実測: 横動画 → `1000×562`、縦動画 → `1000×1778`。**iframe フォールバック経路のみ** Drive のコントロールバー高さ分を `withControlBar` で height に上乗せ (自前 video 経路ではネイティブコントロールが崩れないので余白なし) |
...
| 運用要件 | 両 config example の `[plugins].allowed` に `"google-drive"` (アクティブ形式、NSFW ではない)。**スマホで崩れない自前 video プレイヤーを使うには embed を有効化する** (`[embed].enabled = true` + `[embed].publicUrl` 設定)。`renderEmbed` 実装プラグインなので `[embed].allowedPlugins` の auto-fill 対象になる。embed 無効なら Drive `/preview` iframe にフォールバック (スマホでコントロールが崩れる Drive 側の既知問題あり) |
