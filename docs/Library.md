...
| **domainStrategyCache** | *DomainStrategyCacheOptions* | 経路学習キャッシュ設定 (phase14 Step 1)。host + path prefix 1〜2 段ごとに「成功した取得経路」(`default` / `fallback_ua` / `proxy` / `curl_cffi`) を学習・JSONL 永続化し、次回以降のリクエストで第一選択肢として使う設計。`{ enabled, bootstrapPath?, runtimePath?, maxEntries?, consecutiveFailureThreshold?, compactionThreshold? }`。Fastify モードでは `[scraping.strategy_cache]` から自動注入。**注**: phase14 Step 1 ではストレージ層と TOML 設定パスのみ。`scpaping()` への統合は Step 2 で行うため、現時点では設定しても挙動は変わらない | `undefined` |
